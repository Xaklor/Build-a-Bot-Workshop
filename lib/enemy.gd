...
func _process(delta: float) -> void:
	...
	# if we have a seek target, pick a tile to move towards
		...
		if theta > PI/4:
			move_target = tile_map.map_to_local(Vector2i(grid_pos.x, grid_pos.y - 1))
		elif theta < -PI/4:
			move_target = tile_map.map_to_local(Vector2i(grid_pos.x, grid_pos.y + 1))
		elif seek_target.position.x > position.x:
			move_target = tile_map.map_to_local(Vector2i(grid_pos.x + 1, grid_pos.y))
		else:
			move_target = tile_map.map_to_local(Vector2i(grid_pos.x - 1, grid_pos.y))
...
func get_nearest(classname: String) -> Node2D:
	...
	for object in main.find_children("*", classname):
		var d = get_dist(grid_pos, tile_map.local_to_map(object.position))
		if d < dist:
			dist = d
			result = object
	return result
		...
		match thought.get("condition", ""):
			...
			"in range":
				match thought.get("arg1", ""):
					"robot":
						for robot in main.find_children("*", "Robot"):
							if thought.get("arg2", -1) is float and in_range(grid_pos, tile_map.local_to_map(robot.position), thought.get("arg2", -1)):
								thinking = false
			"x in range":
				match thought.get("arg1", ""):
					"robot":
						var count = 0
						for robot in main.find_children("*", "Robot"):
							if thought.get("arg2", -1) is int and in_range(grid_pos, tile_map.local_to_map(robot.position), thought.get("arg2", -1)):
								count += 1
		...
		match behavior.get("behavior", "idle"):
			...
			"aoe":
				...
				if behavior.get("range", -1) is int:
					for robot in main.find_children("*", "Robot"):
						if in_range(grid_pos, tile_map.local_to_map(robot.position), behavior.get("range", -1)):
